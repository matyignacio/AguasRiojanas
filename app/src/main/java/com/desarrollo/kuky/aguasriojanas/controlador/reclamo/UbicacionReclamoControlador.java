...
public class UbicacionReclamoControlador {

    public void buscarUbicacionesReclamo(Activity a, ProgressBar progressBar, TextView tvProgressBar,
                                         ArrayList<UbicacionReclamo> tipoReclamos, Callable<Void> method) {
        displayProgressBar(a, progressBar, tvProgressBar, "Cargando ubicacion de reclamos...");
        StringRequest request = new StringRequest(Request.Method.POST, VOLLEY_HOST + MODULO_AGUAS_RIOJANAS + "reclamo_ubicacion_select.php", response -> {
            ...
            if (!response.equals("ERROR_ARRAY_VACIO")) {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        UbicacionReclamo ubicacionReclamo = new UbicacionReclamo();
                        ubicacionReclamo.setId(Integer.parseInt(jsonArray.getJSONObject(i).getString("id")));
                        ubicacionReclamo.setNombre(jsonArray.getJSONObject(i).getString("nombre"));
                        tipoReclamos.add(ubicacionReclamo);
                    }
